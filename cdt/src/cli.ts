...
/**
 * main entry of cdt and its extension
 */
export function main(
  ...
  let args: ASBuildArgs = yargs
    .usage(
      "Build tool for AssemblyScript contract.\n\nUsage:\n  ascdt [input file] [options] -- [asc options]"
    )
    .example(
      "ascdt",
      "Build release of 'assembly/index.ts to build/release/packageName.wasm"
    )
    .example("ascdt --target release", "Build a release binary")
    .example("ascdt -- --measure", "Pass argument to 'asc'")
    .option("baseDir", {
      alias: "d",
      type: "string",
